...
namespace Nop.Web.Controllers
{
  public partial class ShoppingCartController : BasePublicController
  {
    ...
    // @JONAS - I think there is a chance I can greatly improve the total quality and speed at which this adds products tot he cart.
    protected virtual IActionResult GetProductToCartDetails(List<string> addToCartWarnings, ShoppingCartType cartType, Product product, int index)
    {
        ...
        default:
          {
            ...
            if (_shoppingCartSettings.DisplayCartAfterAddingProduct || index == -1)
            {
    ...
    protected virtual IActionResult GetAllProductToCartDetails(List<string> addToCartWarnings, ShoppingCartType cartType, List<Product> products)
    {
    ...
    public virtual IActionResult AddProductToCart_Details(int productId, int shoppingCartTypeId, int index, string btnName, IFormCollection form)
    {
      ...
      if (index == -1)
      {
        // @JONAS - quantValue returns all values that are currently in the input text values
        int totalCount = 0;

        string quantValue = Request.Form["enteredQuantity"];
        string btnNames = Request.Form["hiddenID"];

        string[] words = quantValue.Split(',');
        string[] products = btnNames.Split(',');


        int[] quants = new int[words.Length];
        int[] productIds = new int[words.Length];
        ...
        for (int i = 0; i < words.Length; i++)
        {
          ...
          quants[i] = Convert.ToInt32(words[i]);
          productIds[i] = Convert.ToInt32(products[i]);
          if(quants[i] != 0)
          {
            totalCount++;
          }
        ...
        for (int i = 0; i < quants.Length; i++)
        {
          if (quants[i] != 0)
          {
            product = _productService.GetProductById(productIds[i]);
            if (product == null)
            {
              return Json(new
              {
            ...
            customerEnteredPriceConverted = decimal.Zero;
            if (product.CustomerEntersPrice)
            {
              foreach (var formKey in form.Keys)
              {
                if (formKey.Equals($"addtocart_{productIds[i]}.CustomerEnteredPrice", StringComparison.InvariantCultureIgnoreCase))
                {
            ...
            var updated_cartitemid = 0;
            foreach (var formKey in form.Keys)
              if (formKey.Equals($"addtocart_{productIds[i]}.UpdatedShoppingCartItemId", StringComparison.InvariantCultureIgnoreCase))
              {
                int.TryParse(form[formKey], out updated_cartitemid);
                break;
              }
            updatecartitem = null;
            if (_shoppingCartSettings.AllowCartItemEditing && updated_cartitemid > 0)
            {
              ...
              var cart = _workContext.CurrentCustomer.ShoppingCartItems
                   ...
                   .LimitPerStore(_storeContext.CurrentStore.Id)
                   .ToList();
              updatecartitem = cart.FirstOrDefault(x => x.Id == updated_cartitemid);
            ...
            //return result
            // @JONAS - I need to look into making this process faster as of now it slows down the sales process a ton


            //if((iterationCounter + 1) != totalCount)
            //  GetProductToCartDetails(addToCartWarnings, add_cartType, product);


            // @JONAS! 
            // add the product with ajax or some other post method that I have found, this could potenetially be a rough area to get working :0 
            if((iterationCounter + 1) != totalCount)
        ...
        GetAllProductToCartDetails(addToCartWarnings, cartType, productList);
      }
      else
      {
        if (product == null)
        ...
        // @JONAS This is where I can edit the btn click to add all items to cart if there are more than one selected. 
        ...
        foreach (var formKey in form.Keys)
          if (formKey.Equals($"EnteredQuantity", StringComparison.InvariantCultureIgnoreCase))
          {
            ...
            if (product.Id == Convert.ToInt32(name))
            {
              ...
              var supportedNums = results[2].Value.ToList();
      ...
      //return result
      return GetProductToCartDetails(addToCartWarnings, cartType, product, index);
    }
